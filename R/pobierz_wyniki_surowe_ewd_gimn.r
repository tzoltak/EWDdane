...
#' @description
#' Funkcja pobiera dane surowe - w tym wyniki uzyskane za poszczególne kryteria oceny
#' - egzaminu gimnazjalnego i połączone z nimi wyniki surowe sprawdzianu.
#' @param lataEG wektor liczb całkowitych - roczniki egzaminu gimnazjalnego, które mają
#' zostać pobrane (wraz z przypisanymi wynikami sprawdzianu i innymi przydatnymi danymi)
#' @param dlKsztalcenia wektor liczb całkowitych - długość toku kształcenia, czyli
#' z jakim przesunięciem wstecz powinny być pobierane wyniki sprawdzianu
#' @param usunSzkolySpecjalne wartość logiczna - czy usuwać uczniów szkół dla dorosłych,
#' specjalnych, itp.?
#' @param jezykiObce wartość logiczna - czy zwrócone mają być również wyniki w zakresie
#' języków obcych?
#' @param wyliczNormalizacje wartość logiczna - czy jeśli zostanie stwierdzone, że
#' w bazie nie mazapisanych normalizacji ekwikwantylowych wyników egzaminów, to obliczyć
#' je i zapisać do bazy?
#' @param zrodloDanychODBC opcjonalnie nazwa źródła danych ODBC, dającego dostęp do bazy
#' (domyślnie "EWD")
#' @param backupZapisz nazwa pliku .RData, do którego zostaną zapisane dane po pobraniu
#' i scaleniu, ale przed wyliczeniem sum i normalizacją
#' @param backupWczytaj nazwa pliku .RData, zapisanego przy wcześniejszym wywołaniu
#' funkcji z wykorzystaniem argumentu \code{backupZapisz}
#' @details
#' Tu się przyda jakiś opis
#' @return Data frame, który zawiera: i tu też trzeba będzie opisać.
#' @export
pobierz_wyniki_surowe_ewd_gimn = function(lataEG, dlKsztalcenia=3:4,
                                          usunSzkolySpecjalne=TRUE, jezykiObce=FALSE,
                                          wyliczNormalizacje=TRUE, zrodloDanychODBC="EWD",
                                          backupZapisz=NULL, backupWczytaj=NULL) {
